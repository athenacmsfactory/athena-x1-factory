...
- Draai de `athenafier.js` om een basis blueprint, data-extractie en een unified `index.html` te krijgen. (De originele HTML wordt opgeslagen als `index.original.html`):
  ```bash
  node factory/5-engine/athenafier.js [naam]
  ```

### 2. Snel Deployen (De "Quick Wrap" weg)
Als je het prototype direct live wilt zien in de Athena Engine zonder componenten te herschrijven:
```bash
...
De V10 structuur voor sitetypes ziet er als volgt uit:
```
├── 3-sitetypes/           <-- Pure V10 Registry: Blueprints & Configs
...
│       └── [25+ other types...]
```
Ga naar `factory/3-sitetypes/unified/[naam]/web/standard/`.
Je moet hier de AI-code opsplitsen in Athena-logica.

### 4. De Gouden Prompt voor AI (Gemini/Claude)
...
```bash
node 6-utilities/generate-site.js [site-naam] [naam]
```
