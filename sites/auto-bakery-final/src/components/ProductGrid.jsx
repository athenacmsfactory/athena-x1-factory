            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
                    {data.map((item, index) => {
                        ...
                        const titleRes = useLego(item, titleKey, "");
                        ...
                        return (
                            <article key={index} className="flex flex-col bg-surface rounded-[2.5rem] shadow-xl overflow-hidden transition-all hover:-translate-y-2 hover:shadow-2xl group border border-slate-100">
                                ...
                                <div className="p-8 flex flex-col flex-grow text-center">
                                    <h3 
                                        className="text-2xl font-bold mb-4 min-h-[4rem] flex items-center justify-center" 
                                        style={{ color: "#654e3f" }}
                                        {...bindProps(titleRes, sectionName, index, 'text')}
                                    >
                                        {titleRes.content}
                                    </h3>
                                    ...
                                    <div className="flex flex-col gap-3">
                                        {isWebshop && (
                                            <button onClick={() => addToCart({ id: item.id || index, title: titleRes.content, price: priceValue, image: imgRes.content })} className="w-full py-4 bg-[var(--color-button-bg)] text-white rounded-2xl font-bold hover:brightness-110 transition-all shadow-lg active:scale-95 text-lg uppercase tracking-wider">In winkelwagen</button>
                                        )}
                                        {hasSearchLinks && (
                                            <a href={getGoogleSearchUrl(titleRes.content)} target="_blank" rel="noopener noreferrer" className="text-slate-400 hover:text-primary transition-colors text-sm flex items-center justify-center gap-2">
