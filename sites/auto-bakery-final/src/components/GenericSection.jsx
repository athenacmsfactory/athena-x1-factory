                ...
                <div className="max-w-7xl mx-auto">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 auto-rows-[200px]">
                        {data.map((item, index) => {
                            ...
                            const titleRes = titleKey ? useLego(item, titleKey, "") : null;
                            ...
                            return (
                                <div key={index} className={`relative overflow-hidden rounded-2xl shadow-lg group ${index === 0 ? 'md:col-span-2 md:row-span-2' : ''}`}>
                                    ...
                                    />
                                    {titleRes && titleRes.content && (
                                        <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-4">
                                            <p className="text-white font-semibold text-sm" {...bindProps(titleRes, sectionName, index, 'text')}>
                                                {titleRes.content}
                                            </p>
                                        </div>
                                    )}
                ...
                <div className="max-w-7xl mx-auto">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {data.map((item, index) => {
                            ...
                            const imgRes = imgKey ? useLego(item, imgKey, "") : null;
                            const titleRes = titleKey ? useLego(item, titleKey, "") : null;
                            ...
                            return (
                                <div key={index} className="flex flex-col bg-[var(--color-card,#2a2a3e)] rounded-3xl overflow-hidden shadow-xl hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 border border-white/5">
                                    ...
                                    <div className="p-8 flex flex-col flex-1">
                                        ...
                                        {titleRes && titleRes.content && (
                                            <h3 className="text-2xl font-serif font-bold text-[var(--color-title,#fefae0)] mb-3" {...bindProps(titleRes, sectionName, index, 'text')}>
                                                {titleRes.content}
                                            </h3>
                                        )}
                                        {textKeys.map(tk => {
                                            const textRes = useLego(item, tk, "");
                                            return (
                                                <div key={tk} className="text-[var(--color-text,#e0e0e0)]/80 leading-relaxed" {...bindProps(textRes, sectionName, index, 'text')}>
                                                    {textRes.content}
                                                </div>
                                            );
                                        })}
            ...
            <div className="max-w-6xl mx-auto">
                ...
                <div className="space-y-20">
                    {data.map((item, index) => {
                        ...
                        return (
                            <div key={index} className={`flex flex-col ${imgRes && imgRes.content ? (isEven ? 'md:flex-row' : 'md:flex-row-reverse') : ''} gap-12 md:gap-20 items-center`}>
                                {imgRes && imgRes.content && (
                                    <div className="w-full md:w-1/2 aspect-[4/3] rounded-3xl overflow-hidden shadow-2xl">
                                        <img src={getImageUrl(imgRes.content)} {...bindProps(imgRes, sectionName, index, 'image')} className="w-full h-full object-cover" />
                                    </div>
                                )}
                                <div className="flex-1">
                                    {titleRes && titleRes.content && (
                                        <h3 className="text-3xl font-serif font-bold text-[var(--color-title,#fefae0)] mb-4" {...bindProps(titleRes, sectionName, index, 'text')}>
                                            {titleRes.content}
                                        </h3>
                                    )}
                                    {textKeys.map(tk => {
                                        const textRes = useLego(item, tk, "");
                                        return (
                                            <div key={tk} className="text-xl leading-relaxed text-[var(--color-text,#e0e0e0)]/75 mb-4" {...bindProps(textRes, sectionName, index, 'text')}>
                                                {textRes.content}
                                            </div>
                                        );
                                    })}
                                    {(item.link || item.link_url) && (
                                        <a href={item.link_url || item.link} {...bindProps(useLego(item, 'link', item.link || 'Lees meer'), sectionName, index, 'link')}
                                            className="inline-flex items-center gap-2 text-[var(--color-accent,#bc6c25)] font-bold hover:underline text-lg mt-2">
                                            {item.link || 'Lees meer'}
                                        </a>
                                    )}
                                    {hasSearchLinks && titleRes && titleRes.content && (
